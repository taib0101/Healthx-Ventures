...
def createUser(database, cursor, exists, requestedData):
    try:
        ...
        if requestedData != {}:
            ...
            password = bcrypt.hashpw(requestedData["password"].encode(), bcrypt.gensalt()).decode()
...
def readSingleUser_Password(cursor, requestedData):
    ...
    checkHasedPassword = bcrypt.checkpw(requestedData["password"].encode(), password[1].encode())
